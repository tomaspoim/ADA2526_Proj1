            ...
            for(int col = land.columns - 1; col >= 0; col--) {
                ...
                for(int consecutiveJumps = 0; consecutiveJumps < overConsecutiveJumps; consecutiveJumps++) {
                    ...
                    for(int totalJumps = consecutiveJumps; totalJumps < overJumpLimit; totalJumps++) {
                        ...
                        if((tile.type != Tile.Type.JUMP_BARRIER) && consecutiveJumps < land.maxConsecutiveJumps && totalJumps < land.jumpLimit) {
                            ...
                            if(tile.type != Tile.Type.DIAGONAL_BARRIER) {
                                if(!((row + 1 >= land.rows) || (col - 1 >= land.columns) || (row + 1 < 0) || (col - 1 < 0))) {
                                    totalPaths = (totalPaths + dp[row + 1][col - 1][consecutiveJumps + 1][totalJumps + 1]) % MODULO; // LD
                                }
                                if(!((row + 1 >= land.rows) || (col + 1 >= land.columns) || (row + 1 < 0) || (col + 1 < 0))) {
                                    totalPaths = (totalPaths + dp[row + 1][col + 1][consecutiveJumps + 1][totalJumps + 1]) % MODULO; // RD
                                }
                            }
                        }
